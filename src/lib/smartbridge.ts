...
function symbolOnChainForRead(symbol: TokenSymbol, chain: ChainId): TokenSymbol {
  ...
  // Non-Lisk chains cannot read USDCe/USDT0; map to canonical
 ...
 * - Otherwise (Optimism or same-chain) we bridge to the user.
 *
...
export async function ensureLiquidity(
  ...
  // ───────────────────────────────────────────────────────────────
  // PICK SOURCE CHAIN + TOKEN (Optimism / Lisk)
  // ───────────────────────────────────────────────────────────────
  ...
  const candidateSourceTokens: Extract<TokenSymbol, 'USDC' | 'USDT'>[] =
    wantsUsdt0 || wantsUsdce
      ? opts?.preferredSourceToken
        ? [
            opts.preferredSourceToken,
            opts.preferredSourceToken === 'USDC' ? 'USDT' : 'USDC',
          ]
        : ['USDC', 'USDT'] // try both if no preference
      : [symbol as Extract<TokenSymbol, 'USDC' | 'USDT'>]

  // Source chains we can bridge from (no Base)
  const sources: ChainId[] = isLisk ? ['optimism'] : ['optimism', 'lisk']

  // Helper to find a source (chain, token) that has at least `need`
  const findSourceWith = async (
    tok: Extract<TokenSymbol, 'USDC' | 'USDT'>,
    need: bigint,
  ) => {
    const balances: Record<ChainId, bigint> = {} as any
    await Promise.all(
      sources.map(async (c) => {
        try {
    ...
    const ordered = [...sources].sort((a, b) =>
      Number((balances[b] ?? 0n) - (balances[a] ?? 0n)),
    )
    const from = ordered.find((c) => (balances[c] ?? 0n) >= need)
  ...
  if (!picked.from) {
    ...
    for (const t of candidateSourceTokens) {
      ...
      await Promise.all(
        sources.map(async (c) => {
          try {
            balances[c] = await getBalanceOnChain(
              c,
              addressOnChain(t, c),
              user,
            )
      ...
      const bestChain = orderedSources[0]
      totals.push({ token: t, chain: bestChain, bal: balances[bestChain] ?? 0n })
    }
  ...
  const tokenToReceiveOnDest: TokenSymbol = wantsUsdt0
    ? 'USDT0'
    : wantsUsdce
    ? 'USDCe'
    : symbol
  ...
  await bridgeTokens(
    tokenToReceiveOnDest, // receive on Lisk as USDT0/USDCe, or canonical elsewhere
