...
function findSnapshotForPosition(p: PositionLike, snapshots: YieldSnapshot[] | undefined): YieldSnapshot {
...
/**
 * Prefer OP-side "sVault / vault / rewards" balances for deposits when present,
 * otherwise fall back to Morpho Lisk shares.
 */
export function pickEffectiveSharesForToken(
  ...
  const base =
    tokenUpper === 'USDCE' ? 'USDC' : tokenUpper === 'USDT0' ? 'USDT' : tokenUpper

  const sVaultCandidate = allPositions.filter((p) => {
    const proto = String((p as any).protocol ?? '').toLowerCase()
    const chain = String((p as any).chain ?? '').toLowerCase()
    const sym = String((p as any).token ?? '').toUpperCase()

    const looksLikeSVaultProtocol = proto.includes('vault') || proto.includes('svault') || proto.includes('rewards')
    const sameFamily = sym.includes(base) || sym === tokenUpper
    const isOp = chain === 'optimism'
  ...
  const decimals = TOKEN_DECIMALS[token] ?? (token === 'USDCe' || token === 'USDT0' ? 6 : 18)
...
const MyPositions: React.FC<MyPositionsProps> = ({ networkFilter, protocolFilter, filterUI }) => {
  ...
  const positions = useMemo(() => (positionsRaw ?? []) as unknown as PositionLike[], [positionsRaw])
  ...
  const tableData: TableRow[] = useMemo(() => {
    let rows = positionsForMorpho.map((p) => {
      const snap = findSnapshotForPosition(p, snapshots)
      const tokenSymbol = String(p.token) as MorphoToken

      const effective = pickEffectiveSharesForToken(positions, tokenSymbol, (p as any).amount ?? 0n)
      const depositsHuman = formatAmountBigint(effective.shares, effective.decimals)
