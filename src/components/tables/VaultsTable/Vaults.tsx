// src/components/tables/VaultsTable/Vaults.tsx  (SA Implementation)
'use client'
...
const HARD_FILTER = (y: Pick<YieldSnapshot, 'chain' | 'protocolKey' | 'token'>) =>
  y.chain === 'lisk' && y.protocolKey === 'morpho-blue' && (y.token === 'USDC' || y.token === 'USDT')
...
const Vaults: React.FC<VaultsProps> = ({ networkFilter, protocolFilter, filterUI }) => {
  ...
  const data = useMemo(() => {
    ...
    let mapped = filtered.map((snap) => {
      ...
      const routeKey =
        snap.token === 'USDC' ? 'USDCe' : snap.token === 'USDT' ? 'USDT0' : snap.token
      ...
      return {
        ...
        tvl: Number.isFinite(snap.tvlUSD) ? Math.round(snap.tvlUSD).toLocaleString() : '0',
      }
    })
